...
import {
  ...
  Search, Shield, MapPin, Phone, Calendar, ChevronRight, X, Info, FileSpreadsheet, Server
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [inputResponsabilidad, setInputResponsabilidad] = useState('Patrullado');
  ...
  const [editResponsabilidad, setEditResponsabilidad] = useState('');
  ...
  const closeAddModal = () => {
    ...
    setFoundPerson(null);
    setInputTelefono('');
    setInputResponsabilidad('Patrullado');
    setAddError('');
  };
    ...
    try {
      const res = await fetch('/api/patrulla/integrantes', {
        ...
        body: JSON.stringify({
          ...
          responsabilidad: editResponsabilidad
        })
      });
      const data = await res.json();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-slate-400">Cargando sistema 1x10...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-16">
      ...
      {/* Toast Notificación */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3 border transition-all animate-bounce ${
      ...
      <header className="sticky top-0 z-30 bg-slate-900/90 backdrop-blur-md border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-blue-500 flex items-center justify-center font-black text-white shadow-lg shadow-indigo-600/30">
              1x10
            </div>
            <div>
              <h1 className="font-bold text-white text-base leading-tight">Sistema Popular 1x10</h1>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold border border-slate-700 transition-colors"
            >
              <Server className="w-3.5 h-3.5 text-indigo-400" />
              <span>Despliegue CloudPanel</span>
            </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        {/* Tarjeta de Información del Jefe y Progreso del 1x10 */}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-indigo-950 text-indigo-300 border border-indigo-800/80">
                  JEFE DE PATRULLA
                </span>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-slate-900/90 p-4 rounded-xl border border-slate-800">
              <div className="space-y-1">
                <div className="flex items-center justify-between gap-6 text-xs text-slate-400 font-semibold">
                  <span>META 1x10 DE PATRULLA</span>
                  <span className="text-indigo-400">{integrantesCount} / 10</span>
                </div>
                <div className="w-48 sm:w-56 h-3 bg-slate-800 rounded-full overflow-hidden p-0.5 border border-slate-700">
                  <div
        ...
        {/* Listado de Integrantes (Grid de 10 Cupos) */}
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              <Users className="w-5 h-5 text-indigo-400" />
              <span>Integrantes de la Patrulla (1x10)</span>
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
            ...
            {/* Iterar sobre los integrantes registrados */}
            {jefe?.integrantes.map((member, idx) => (
              <div
                key={member.id}
                className="glass-card rounded-xl p-5 relative group border border-slate-800 hover:border-indigo-500/50 transition-all"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3.5">
                    <div className="w-9 h-9 rounded-lg bg-indigo-950/80 border border-indigo-800 text-indigo-300 font-extrabold flex items-center justify-center text-sm shrink-0">
                      #{idx + 1}
                    </div>
                    <div>
                      <h4 className="font-bold text-white text-base leading-snug">{member.nombre}</h4>
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        <span className="text-xs font-semibold text-slate-300 bg-slate-900 px-2 py-0.5 rounded border border-slate-800">
                          {member.cedula}
                        </span>
                        <span className="text-[11px] font-medium text-emerald-400 bg-emerald-950/80 px-2 py-0.5 rounded border border-emerald-900">
                      ...
                      <div className="mt-3 space-y-1 text-xs text-slate-400">
                        ...
                        <div className="flex items-center gap-1.5">
                          <MapPin className="w-3.5 h-3.5 text-slate-500" />
                          <span>{member.comunidad || jefe.comunidad}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 opacity-90 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => {
                        ...
                        setEditResponsabilidad(member.responsabilidad || 'Patrullado');
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg bg-red-950/60 hover:bg-red-900/80 text-red-400 hover:text-red-200 transition-colors border border-red-900"
                      title="Eliminar de la patrulla"
                    >
            ...
            {/* Renderizar ranuras vacías hasta completar los 10 cupos */}
            {Array.from({ length: 10 - integrantesCount }).map((_, idx) => (
              <div
                key={`empty-${idx}`}
                onClick={() => setIsAddModalOpen(true)}
                className="rounded-xl p-5 border-2 border-dashed border-slate-800/80 hover:border-indigo-500/50 bg-slate-900/30 hover:bg-slate-900/60 transition-all cursor-pointer flex items-center justify-between group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-slate-900 text-slate-600 font-bold flex items-center justify-center text-sm border border-slate-800">
                    #{integrantesCount + idx + 1}
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-slate-400 group-hover:text-indigo-300 transition-colors">
                      Cupo Vacante #{integrantesCount + idx + 1}
                    </div>
                    <div className="text-xs text-slate-600">Haga clic para ingresar integrante</div>
                  </div>
                </div>
      ...
      {/* ======================================================== */}
      {/* MODAL 1: BUSCAR E INGRESAR INTEGRANTE POR CÉDULA */}
      {/* ======================================================== */}
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="glass-panel w-full max-w-lg rounded-2xl p-6 border border-slate-800 shadow-2xl relative">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">Ingresar Integrante 1x10</h3>
                <p className="text-xs text-slate-400">Paso 1: Verificar cédula en el padrón electoral Excel</p>
              </div>
            </div>
              ...
              <form onSubmit={handleVerifyCedula} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full pl-4 pr-10 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500"
                    />
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold flex items-center gap-2 shadow-lg shadow-indigo-600/30"
                  >
                    {verifyingCedula ? 'Verificando en Excel...' : 'Buscar en Padrón'}
                  </button>
                </div>
              </form>
            ) : (
              /* Confirmación de Datos del Ciudadano Encontrado en Excel */
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-slate-900/90 border border-emerald-800/60 text-xs space-y-2">
                  <div className="flex items-center gap-2 text-emerald-400 font-bold">
                    <CheckCircle2 className="w-4 h-4" />
                    <span>Registro encontrado en Padrón Electoral</span>
                  </div>
                  ...
                  <div className="text-slate-400 grid grid-cols-2 gap-1 pt-1 border-t border-slate-800">
                    ...
                    <div className="col-span-2">Comunidad: <span className="text-slate-200">{foundPerson.comunidad}</span></div>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Responsabilidad / Rol</label>
                  <select
                    value={inputResponsabilidad}
                    onChange={(e) => setInputResponsabilidad(e.target.value)}
                    className="w-full px-3.5 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white text-sm"
                  >
                    <option value="Patrullado">Patrullado / Integrante Base</option>
                    <option value="Logística">Responsable de Logística</option>
                    <option value="Movilización">Responsable de Movilización</option>
                    <option value="Comunicación">Responsable de Comunicación</option>
                  </select>
                </div>
      ...
      {isEditModalOpen && selectedMember && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md rounded-2xl p-6 border border-slate-800 shadow-2xl relative">
            ...
            <h3 className="text-lg font-bold text-white mb-1">Editar Integrante</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Responsabilidad</label>
                <select
                  value={editResponsabilidad}
                  onChange={(e) => setEditResponsabilidad(e.target.value)}
                  className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white text-sm"
                >
                  <option value="Patrullado">Patrullado / Integrante Base</option>
                  <option value="Logística">Responsable de Logística</option>
      ...
      {isDeleteModalOpen && selectedMember && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-sm rounded-2xl p-6 border border-slate-800 shadow-2xl text-center">
            <div className="w-12 h-12 rounded-full bg-red-950 text-red-400 flex items-center justify-center mx-auto mb-3 border border-red-900">
              <Trash2 className="w-6 h-6" />
            </div>
            <h3 className="text-base font-bold text-white">¿Eliminar Integrante?</h3>
            <p className="text-xs text-slate-400 mt-1">
              ¿Está seguro de eliminar a <strong className="text-slate-200">{selectedMember.nombre}</strong> de su patrulla 1x10? Esta acción liberará el cupo.
            </p>
